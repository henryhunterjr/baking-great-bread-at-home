...
import { RecipeData, ConversionResult, MeasurementSystem, RecipeType } from './types';
...
export abstract class BaseConverter {
  ...
  async convert(recipeData: RecipeData, targetSystem: MeasurementSystem): Promise<ConversionResult> {
    ...
    const bakersPercentages = calculateBakersPercentages(converted.ingredients);
    const hydration = calculateHydration(converted.ingredients);
